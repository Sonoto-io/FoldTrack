...
const onFormSubmit = async (event: FormSubmitEvent) => {
  ...
  if (Object.keys(event.errors).length === 0) {
    const newPassword = event.values.password
    const { data, error } = await resetPasswordWithToken(newPassword)
    if (error) {
      const authError = mapSupabaseAuthError(error)
      toast.add({
        severity: 'error',
        summary: 'Error when updating passwords',
    ...
    toast.add({
      ...
      detail: 'Your password has been updated successfully. Redirecting to login...',
      life: 3000,
    })
